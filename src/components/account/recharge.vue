<template>
  <div class="recharge">
    <PageTitle :tips="tips">
      ...
      <template slot="path">充值</template>
      <template slot="mainTitle">充值</template>
    </PageTitle>
    ...
    <div class="selectRecharge">
      <div class="amount">
        <RadioGroup v-model="amount" vertical>
          <template>
            <Radio v-for="(item, index) in ammountList" :key="index" :label="item">
              <span>{{item}}</span>
            </Radio>
          </template>
          <Radio label="-1">
            <span>自定义充值</span>
            <InputNumber v-model="amountSecond" :editable="true" :step="1" :precision="precision"></InputNumber>
          </Radio>
        </RadioGroup>
      </div>
      <div class="way">
      ...
      <div class="confirm">
        <div class="confirmBtn" @click="confirmRecharge">立即充值</div>
      </div>
    </div>
  </div>
</template>
...
import { jsPost } from '../../static/js/config.js'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    unionPay(balance) {
      window.location.href =
        this.$global.baseURL +
        '/frontend/user_wallet/recharge?session_id=' +
        Cookies.get('session') +
        '&type=2&balance=' +
        balance
    },
    aliPay(balance) {
      http
        .postNormal('/frontend/user_wallet/recharge', {
          type: 1,
          balance
        })
        .then(res => {
